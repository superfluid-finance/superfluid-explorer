...
export class LandingPage extends BasePage {
  ...
  static openDataPage(page: string, network: string) {
    switch (page) {
        ...
        cy.fixture('accountData').then((fixture) => {
          cy.visit(
            '/' +
              network +
              '/accounts/' +
              fixture[network].gdaAdminAccount.address
          )
        })
        break
      }
        ...
        cy.fixture('accountData').then((fixture) => {
          cy.visit(
            '/' +
              network +
              '/accounts/' +
              fixture[network].gdaMemberAccount.address
          )
        })
        break
      }
        ...
        cy.fixture('gdaData').then((fixture) => {
          cy.visit(
            '/' +
              network +
              '/pool-members/' +
              fixture[network].poolMembers[0].id
          )
        })
        break
      }
      ...
      case 'second balance account for filtering': {
        cy.fixture('filteringData').then((fixture) => {
          cy.visit(
            '/' + network + '/accounts/' + fixture[network].secondAddress
          )
        })
        break
      }
        ...
        cy.fixture('tokenData').then((fixture) => {
          cy.visit(
            '/' +
              network +
              '/supertokens/' +
              fixture[network].regularTokenAddress
          )
        })
        break
      }
  ...
  static validateLatestStreamHyperlinks(network: string) {
    cy.get(ADDRESS_LINKS)
      .should('have.attr', 'href')
      .and('contain', network + '/accounts/')
    cy.get(TOKEN_LINKS)
      .should('have.attr', 'href')
      .and('contain', network + '/supertokens/')
  }
