...
const TOTAL_AMOUNT_RECEIVED = "[data-cy=subscription-total-amount-received] span"
...
export class IndexSubscriptionPage extends BasePage {
  ...
  static validateSubscriptionGeneralInfo(network: string) {
    cy.fixture("accountData").then(fixture => {
      cy.wrap(fixture[network].superApp.indexes.publications[0].details.subscriptions[0].details).then((subscription: any) => {
  ...
  static validateSubscriptionUnitsInfo(network: string) {
    cy.fixture("accountData").then(fixture => {
      cy.wrap(fixture[network].superApp.indexes.publications[0].details.subscriptions[0].details).then((subscription: any) => {
        ...
        this.containsText(TOTAL_AMOUNT_RECEIVED, subscription.totalAmountReceived)
      })
    })
  }
  ...
  static validateSubscriptionUnitsUpdated(network: string) {
    cy.fixture("accountData").then(fixture => {
      fixture[network].superApp.indexes.publications[0].details.subscriptions[0].details.unitsUpdated.forEach((unitsUpdate: any, index: number) => {
        cy.get(UNITS_UPDATED).eq(index).should("contain.text", unitsUpdate.units)
      })
    })
  }
}
