...
const STREAM_PERIOD_TOTAL_STREAMED =
  '[data-cy=stream-period-grid] [data-cy=total-streamed]'
...
export class StreamsPage extends BasePage {
  ...
  static validateStreamPeriods(network: string) {
    cy.fixture('streamData').then((streamData) => {
      streamData[network].streamPeriods.forEach(
        (period: any, index: number) => {
          cy.get(STREAM_PERIOD_FLOW_RATES)
            .eq(index)
            .should('have.text', period.flowRate)
          cy.get(STREAM_PERIOD_TOTAL_STREAMED)
            .eq(index)
            .should('have.text', period.totalStreamed)
        }
      )
    })
  }
